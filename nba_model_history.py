...
DB_PATH = Path(
    os.environ.get(
        "FIRST_FIVE_EDGE_NBA_HISTORY_DB",
        Path(tempfile.gettempdir()) / "first_five_edge_nba_model_history.sqlite3",
    )
)
...
def init_db(connection):
    connection.execute(
        """
        CREATE TABLE IF NOT EXISTS nba_model_history (
    ...
    columns = {
        row["name"]
        for row in fetch_rows(connection, "PRAGMA table_info(nba_model_history)")
    }
    if "locked_at" not in columns:
        connection.execute("ALTER TABLE nba_model_history ADD COLUMN locked_at TEXT")
    if "snapshot_status" not in columns:
        connection.execute(
            "ALTER TABLE nba_model_history ADD COLUMN snapshot_status TEXT"
        )
    connection.commit()
...
def insert_prediction(connection, row_values):
    ...
    connection.execute(
        f"""
        INSERT INTO nba_model_history ({", ".join(columns)})
        VALUES ({placeholders})
        """,
...
def load_nba_history(model_version=None, market_version=None, db_path=DB_PATH):
    with connect(db_path) as connection:
        init_db(connection)
        where = []
        params = []
...
def load_nba_performance_summary(
    model_version=None,
    market_version=None,
    db_path=DB_PATH,
):
    rows = load_nba_history(model_version, market_version, db_path)
    side_signals = [
        row
        for row in rows
        if row.get("side_edge") not in [None, "", "Pass"]
...
def load_nba_performance_tables(
    model_version=None,
    market_version=None,
    db_path=DB_PATH,
):
