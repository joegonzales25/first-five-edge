...
def fetch_nba_scoreboard(start_date: date, end_date: date, limit: int = 500) -> dict:
...
def normalize_nba_events(events: list[dict]) -> pd.DataFrame:
    rows = []
    for event in events:
...
def nba_season_start(today: date) -> date:
    season_year = today.year if today.month >= 9 else today.year - 1
    return date(season_year, 10, 1)


def load_nba_current_season(
    season: int | None = None,
    today: date | None = None,
    days_ahead: int = 14,
) -> pd.DataFrame:
    today = today or date.today()
    start_date = date(season, 10, 1) if season else nba_season_start(today)
    end_date = today + timedelta(days=days_ahead)
    data = fetch_nba_scoreboard(start_date, end_date)
    return normalize_nba_events(data.get("events", []))
