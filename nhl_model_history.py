...
DB_PATH = Path(
    os.environ.get(
        "FIRST_FIVE_EDGE_NHL_HISTORY_DB",
        Path(tempfile.gettempdir()) / "first_five_edge_nhl_model_history.sqlite3",
    )
)
TABLE_NAME = "nhl_model_history"
...
def result_values(row, now):
    ...
    total_error = None
    if safe_float(row.get("Projected Total")) is not None and actual_total is not None:
        total_error = abs(safe_float(row.get("Projected Total")) - actual_total)
...
def record_nhl_history(slate, slate_date, market_version, model_version, db_path=DB_PATH):
...
def load_nhl_history(model_version=None, market_version=None, db_path=DB_PATH):
    with connect(db_path) as connection:
        init_db(connection)
        where = []
        params = []
...
def performance_row(label, rows):
    side_signals = [
        row for row in rows if row.get("side_edge") not in [None, "", "Pass"]
    ]
    ...
    return {
        ...
        "Moneyline Signals": len(side_signals),
        "Moneyline Accuracy": accuracy(side_signals, "side_result"),
        "Margin MAE": avg(row.get("margin_error") for row in completed),
        "Total MAE": avg(row.get("total_error") for row in completed),
    }


def load_nhl_performance_summary(model_version=None, market_version=None, db_path=DB_PATH):
    rows = load_nhl_history(model_version, market_version, db_path)
    side_signals = [
        row for row in rows if row.get("side_edge") not in [None, "", "Pass"]
    ]
    completed = [row for row in rows if str(row.get("status", "")).lower() == "final"]
...
def load_nhl_performance_tables(model_version=None, market_version=None, db_path=DB_PATH):
    rows = load_nhl_history(model_version, market_version, db_path)
    if not rows:
        return {
            "core": [],
    ...
    established = [
        row
        for row in rows
        if min(safe_int(row.get("away_prior_games")) or 0, safe_int(row.get("home_prior_games")) or 0) >= 5
    ]
    limited = [
        row
        for row in rows
