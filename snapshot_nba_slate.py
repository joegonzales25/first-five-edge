...
from nba_agent import build_current_slate
from nba_model_history import record_nba_history
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Record the current NBA slate snapshot for Edge Detector."
    )
    parser.add_argument(
        "--date",
    ...
    parser.add_argument(
        ...
        help=f"NBA market release. Defaults to {DEFAULT_MARKET_VERSION}.",
    )
    parser.add_argument(
        "--model-version",
        default=DEFAULT_MODEL_VERSION,
        help=f"NBA model baseline. Defaults to {DEFAULT_MODEL_VERSION}.",
    )
...
def main():
    ...
    if slate is None or slate.empty:
        print(f"No NBA games found for {slate_date}.")
        return 0

    counts = record_nba_history(
        slate,
        slate_date,
        args.market_version,
        args.model_version,
    ...
    print(
        f"Recorded NBA snapshot for {slate_date}: "
