...
from nhl_agent import build_current_slate
from nhl_model_history import record_nhl_history
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Record the current NHL slate snapshot for Edge Detector."
    )
    parser.add_argument(
        "--date",
    ...
    parser.add_argument(
        ...
        help=f"NHL market release. Defaults to {DEFAULT_MARKET_VERSION}.",
    )
    parser.add_argument(
        "--model-version",
        default=DEFAULT_MODEL_VERSION,
        help=f"NHL model baseline. Defaults to {DEFAULT_MODEL_VERSION}.",
    )
...
def main():
    ...
    if slate is None or slate.empty:
        print(f"No NHL games found for {slate_date}.")
        return 0

    counts = record_nhl_history(
        slate,
        slate_date,
        args.market_version,
        args.model_version,
    ...
    print(
        f"Recorded NHL snapshot for {slate_date}: "
