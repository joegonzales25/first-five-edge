...
DB_PATH = Path(
    os.environ.get(
        "FIRST_FIVE_EDGE_CBB_HISTORY_DB",
        Path(tempfile.gettempdir()) / "first_five_edge_cbb_model_history.sqlite3",
    )
)
...
def init_db(connection):
    connection.execute(
        """
        CREATE TABLE IF NOT EXISTS cbb_model_history (
    ...
    columns = {
        row["name"]
        for row in fetch_rows(connection, "PRAGMA table_info(cbb_model_history)")
    }
    if "locked_at" not in columns:
        connection.execute("ALTER TABLE cbb_model_history ADD COLUMN locked_at TEXT")
    if "snapshot_status" not in columns:
        connection.execute(
            "ALTER TABLE cbb_model_history ADD COLUMN snapshot_status TEXT"
        )
    connection.commit()
...
def insert_prediction(connection, row_values):
    ...
    connection.execute(
        f"""
        INSERT INTO cbb_model_history ({", ".join(columns)})
        VALUES ({placeholders})
        """,
...
def load_cbb_history(model_version=None, market_version=None, db_path=DB_PATH):
    with connect(db_path) as connection:
        init_db(connection)
        where = []
        params = []
...
def load_cbb_performance_summary(
    model_version=None,
    market_version=None,
    db_path=DB_PATH,
):
    rows = load_cbb_history(model_version, market_version, db_path)
    side_signals = [
        row
        for row in rows
        if row.get("side_edge") not in [None, "", "Pass"]
...
def load_cbb_performance_tables(
    model_version=None,
    market_version=None,
    db_path=DB_PATH,
):
