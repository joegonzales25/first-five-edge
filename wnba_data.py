...
def team_label(competitor: dict) -> str:
    ...
    return team.get("abbreviation") or team.get("shortDisplayName") or team.get("displayName")
...
def normalize_wnba_events(events: list[dict]) -> pd.DataFrame:
    rows = []
    for event in events:
...
def load_wnba_current_season(
    ...
    start_date = date(season, 5, 1)
    end_date = today + timedelta(days=days_ahead)
    data = fetch_wnba_scoreboard(start_date, end_date)
    return normalize_wnba_events(data.get("events", []))
