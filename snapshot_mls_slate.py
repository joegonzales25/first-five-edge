...
from mls_model_history import record_mls_history
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Record the current MLS slate snapshot for Edge Detector."
    )
    parser.add_argument(
        "--date",
    ...
    parser.add_argument(
        ...
        help=f"MLS market release. Defaults to {DEFAULT_MARKET_VERSION}.",
    )
    parser.add_argument(
        "--model-version",
...
def record_slate_date(slate_date, args):
    ...
    print(
        f"Recorded MLS snapshot for {slate_date}: "
