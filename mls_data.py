...
def fetch_mls_scoreboard(start_date: date, end_date: date, limit: int = 500) -> dict:
...
def normalize_mls_events(events: list[dict]) -> pd.DataFrame:
    rows = []
    for event in events:
...
def mls_season_start(today: date) -> date:
    return date(today.year, 2, 1)


def load_mls_current_season(
    season: int | None = None,
    today: date | None = None,
    days_ahead: int = 14,
) -> pd.DataFrame:
    today = today or date.today()
    start_date = date(season, 2, 1) if season else mls_season_start(today)
    end_date = today + timedelta(days=days_ahead)
    data = fetch_mls_scoreboard(start_date, end_date)
    return normalize_mls_events(data.get("events", []))
